...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/nejcoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    NEJCOIND_EXEC=nejcoind.exe
fi
...
get_tag
popd
echo "var GUI_NEJCOIN_VERSION = \"$TAGNAME\"" >> version.js
...
cp ../$NEJCOIN_DIR/bin/$NEJCOIND_EXEC $BIN_PATH
fi
